  ...
  const Account = () => {
    ...
    const handleChange = (event) => {
      ...
      if(event.target.value<=0){setValidTransaction(false)
        ...
        mensaje= `Te falta ${event.target.value-totalState}`;
    ...
    return (
      <form onSubmit={handleSubmit}>
        <h2 id="total">{status}</h2>
          ...
          {validChoice && <div><ATMDeposit onChange={handleChange} isDeposit={isDeposit} isValid = {validTransaction}></ATMDeposit></div>}
